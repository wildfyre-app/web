...
describe('RegistrationService', () => {
  ...
  it('Expected values from first password step', () => {
    ...
    expect(service.recoverPasswordStep1(
      recoverTransaction.email,
      recoverTransaction.username,
      recoverTransaction.captcha
    )).toEqual(new Observable<RecoverTransaction>());
  });

  it('Expected values from second password step', () => {
    spyOn(service, 'recoverPasswordStep2').and.returnValue(new Observable<Reset>());
    expect(service.recoverPasswordStep2(
      reset.new_password,
      reset.token,
      reset.transaction,
      reset.captcha
    )).toEqual(new Observable<Reset>());
  });
  ...
  it('Expected values from registration', () => {
    ...
    )).toEqual(new Observable<Registration>());
  });
});
